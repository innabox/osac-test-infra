...
- `scripts/rotate-slack-oncall.sh` — picks `(whole weeks elapsed since a fixed reference Wednesday) % len(rotation list)` as the index into `SLACK_ONCALL_ROTATION_LIST` (not the ISO week number itself, which resets from 52/53 back to 1 every year and would otherwise pick the same person twice at that boundary for some list lengths), updates `SLACK_ONCALL_USERGROUP_ID`'s sole member via `usergroups.users.update`, calls `users.info` only to get a display name for the message text, and posts the new on-call (plus the Wed-through-Tue duty window) to `SLACK_ONCALL_CHANNEL_ID`. Stateless by design: the index and window are derived from the most recently started Wednesday on or before "today", so a missed/re-run week or a manual run on a non-Wednesday still self-corrects to the right person and dates instead of drifting. Logs and the posted message's display name never surface an email address (the rotation list holds opaque Slack user IDs, not emails), which matters since this repo is public and Actions logs are visible to anyone
- Required secrets: `SLACK_ONCALL_BOT_TOKEN` (scopes: `chat:write`, `usergroups:write`, `users:read`), `SLACK_ONCALL_USERGROUP_ID`, `SLACK_ONCALL_CHANNEL_ID`, `SLACK_ONCALL_ROTATION_LIST` (comma-separated Slack user IDs, e.g. `U0123ABCDEF` -- not email addresses)
